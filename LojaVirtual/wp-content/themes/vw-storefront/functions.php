...
function vw_storefront_activation_notice() {
	...
	echo '<p><a href="'. esc_url( admin_url( 'themes.php?page=vw_storefront_guide' ) ) .'" class="button button-primary">'. esc_html__( 'GET STARTED', 'vw-storefront' ) .'</a></p>';
	echo '</div>';
}
...
function vw_storefront_scripts() {	
	...
	wp_enqueue_script( 'vw-storefront-custom-scripts-jquery', esc_url(get_template_directory_uri()) . '/assets/js/custom.js', array('jquery'),'' ,true );
...
if ( ! function_exists( 'vw_storefront_credit' ) ) {
	function vw_storefront_credit(){
		echo "<a href=".esc_url(VW_STOREFRONT_CREDIT).">".esc_html__('Storefront WordPress Theme','vw-storefront')."</a>";
	}
}
